...
class MapGenerate():
    ...
    def points_to_idx(self, xy=None):
        # convert 3d points xy (Nx3) into indices in occupancy grid
        # @input:   self         self generated by load_map
        #           xy         Nx3 3d points in actual space
        # @output:  ij         Nx3 indices in self's occupancy grid
        ij = np.ones((xy.shape[0], 2), dtype=int)
        if xy.shape[0] > 0:
            for i in range(0,ij.shape[0]):
                ij[i, 0] = int(np.floor((xy[i, 0] - self.boundary[0, 0]) / self.xy_res))
                ij[i, 1] = int(np.floor((xy[i, 1] - self.boundary[1, 0]) / self.xy_res))
        else:
            raise Exception('Wrong!')
        return ij